...
from Clouds_additional_features.ActionsAdd import Parametrs
...
class CloudsTest(unittest.TestCase, Parametrs):
    ...
    #additionFeat = "_Smoke"
    cloudURL = "NULL"
    minFileLim = maxFileLim = 0
    presTime = str(datetime.now().strftime("_%Y-%m-%d_(%H.%M)"))
    pathP = Parametrs().getPath()
    #cycleP = Parametrs().getCycle()
    scanCloud = Parametrs().askCallsCloud()
    scanAdditional = Parametrs().additionalfeatures()
    if scanAdditional == 2:
        minFileLim = input("Input minimal file size limit(KB): ")
        maxFileLim = input("Input maximal file size limit(KB): ")
    ...
    def test_BOX(self):
        ...
        CloudsTest().sTime(2)
        driver.find_element_by_id("textfield-1025-inputEl").send_keys("ATest_", CloudsTest.activeCloud, CloudsTest.checkAdditionFeat(self), CloudsTest.presTime)
        ...
        driver.find_element_by_id("file_scan_limit-inputEl").send_keys("10")
            ...
            #Additionall sets
        if CloudsTest.scanAdditional == 1:
            CloudsTest.additionFeat = "Incremental_Scan"
            driver.find_element_by_id("IncrementalScan-inputEl").click()
        elif CloudsTest.scanAdditional == 2:
            CloudsTest.additionFeat = "File_size_limits"
            driver.find_element_by_id("MinSizeLimitEnabled-inputEl").click()
            driver.find_element_by_id("MinSizeLimit-inputEl").send_keys(CloudsTest.minFileLim)
            driver.find_element_by_id("MaxSizeLimitEnabled-inputEl").click()
            driver.find_element_by_id("MaxSizeLimit-inputEl").send_keys(CloudsTest.maxFileLim)
        elif CloudsTest.scanAdditional == 3:
            CloudsTest.additionFeat = "Files_changed_Scan"
        else:
            sys.exit(3)
...
##################################################_additionals functions_########################################
    def checkAdditionFeat(self):
        if CloudsTest.scanAdditional == 1:
            return "_Incremental_Scan"
        elif CloudsTest.scanAdditional == 2:
            return "_File_size_limits"
        elif CloudsTest.scanAdditional == 3:
            return "_Files_changed_Scan"
        else:
            return "_Smoke"
